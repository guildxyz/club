...
const getBalanceOf =
  (contract: Contract, merkleVestingContractAddress: string) => () =>
    contract?.balanceOf(merkleVestingContractAddress).catch((error) => {
...
const useWithdrawAmount = () => {
  ...
  const swrResponse = useSWR<any>(
    active ? ["vestingWithdrawAmount", chainId, account] : null,
    getBalanceOf(
      rewardTokenContract,
      process.env.NEXT_PUBLIC_MERKLE_VESTING_CONTRACT_ADDRESS
    ),
    {
